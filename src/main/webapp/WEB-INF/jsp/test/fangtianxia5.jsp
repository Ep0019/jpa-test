...
<%@include file="/common/taglibs.jsp"%>
	...
	<script type="text/javascript" src="${ctx}/javascript/InfoBox_min.js"></script>
...
<body>
	<div id="l-map"></div>
	<input type="button" value="银行" onclick="searchInBounds('银行')"/>
	<input type="button" value="公交" onclick="searchInBounds('公交')"/>
	<input type="button" value="教育" onclick="searchInBounds('教育')"/>
	<input type="button" value="生活" onclick="searchInBounds('生活')"/>
	<br>
...
var search_options={
		pid:0,//上级id
		level:1,//层级
		isSearch:false,//是否是在搜索数据
		searchCount:0,//第一次点击搜索
		keywork:"",//关键字检索
		minlat:"",//纬度
		maxlat:"",
		minlng:"",//经度
...
function initData(map,data){
	...
	for(var i=0;i<data.length;i++){
		...
		var myCompOverlay = new ComplexCustomOverlay(point,data[i].id,data[i].pid,data[i].level,data[i].title,data[i].avgprice);
	    	...
	    	var mark = new BMap.Marker(point,{icon:icondefault});
	        	 function showInfo() {
		            ...
		            var infoBox = new BMapLib.InfoBox(map,content,{
		            	boxStyle:{
		            		/* background:"url('${ctx}/upload/tipbox.gif') no-repeat center top", */
		            		width: '200px'
		                    },
		                    closeIconMargin: "2px 2px 0 0",
		                    closeIconUrl:"${ctx}/upload/icon2.png",
		                   /*  enableAutoPan: true, */
		                    alignBottom: false
		 	    	});
...
/* 搜索可视区域的百度地图数据*/
function searchInBounds(content){
	map.clearOverlays();
