...
//操作类型：  1.拖拽加载数据，2.级别放大加载数据，3.点击搜索加载数据，4.点击一二级别加载数据
var pid=0;
var type="";
...
var datax=[
           ...
           {"title":"五缘湾道","avgprice":3.2,"address":"福建省厦门市湖里区五缘湾道","location":"118.174739|24.524561","pid":2,"id":10,"level":1},
...
//三级地区数据
var dataxq=[
           {"title":"特房五缘湾尊府","avgprice":3.2,"address":"福建省厦门市湖里区金山路","location":"118.170984|24.524857","pid":3,"id":100,"level":2},
           {"title":"国贸天琴湾","avgprice":3.2,"address":"福建省厦门市湖里区五缘湾道","location":"118.176445|24.525087","pid":3,"id":200,"level":2},
           {"title":"紫金家园","avgprice":3.2,"address":"福建省厦门市湖里区金山路","location":"118.171092,24.526862","pid":3,"id":300,"level":2}
];
//拖拽数据level=1
var datat1=[
...
ComplexCustomOverlay.prototype.initialize = function(map){
  ...
  //区域块点击事件监听（根据dataId去搜索该区域下的小区域）
  div.onclick = function(){
	  if(dataId == 2){
		  //加载该一级下的所有二级地区数据
		  //datax需要替换成自己的数据（根据pid=dataId的数据获取）
		  map.clearOverlays();
		  initData(map,datax,mylevel++);
	  }else if(dataId == 10){
		  //加载该一级下的所有二级地区数据
		  //datax需要替换成自己的数据（根据pid=dataId的数据获取）
		  map.clearOverlays();
		  initData(map,dataxq,mylevel++);
	  }else{
		  alert("具体数据:"+dataId);
	  }
  }
...
//覆盖物构造函数
function ComplexCustomOverlay(point,dataId,text,mouseoverText,level){
  this._point = point;
  this._text = text;
...
function initData(map,data,level){
	...
	for(var i=0;i<data.length;i++){
		...
		var myCompOverlay = new ComplexCustomOverlay(point,data[i].id,data[i].title,data[i].title+" "+data[i].avgprice,level);
...
content:非必填 搜索内容
*/
function getData(dataId,level,content){
